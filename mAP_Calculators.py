...
# argparse receiving list of classes with specific IoU (e.g., python mAP_Calculators.py --set-class-iou person 0.7)
...
with open(output_files_path + "/output.txt", 'w') as output_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, detection in enumerate(dr_data):
            ...
            poly1 = Polygon(np.array(bb).reshape(4, 2)).convex_hull
            for obj in ground_truth_data:
                # look for a class_name match
                if obj["class_name"] == class_name:
                    bbgt = [float(x) for x in obj["bbox"].split()]
                    poly2 = Polygon(np.array(bbgt).reshape(4, 2)).convex_hull
            ...
            if show_animation:
                ...
                if ovmax > 0:  # if there is intersections between the bounding-boxes
                    ...
                    cv2.line(img, (bbgt[0], bbgt[1]), (bbgt[2], bbgt[3]), light_blue, 2)
                    cv2.line(img, (bbgt[2], bbgt[3]), (bbgt[4], bbgt[5]), light_blue, 2)
                    cv2.line(img, (bbgt[4], bbgt[5]), (bbgt[6], bbgt[7]), light_blue, 2)
                    cv2.line(img, (bbgt[6], bbgt[7]), (bbgt[0], bbgt[1]), light_blue, 2)
                    cv2.line(img_cumulative, (bbgt[0], bbgt[1]), (bbgt[2], bbgt[3]), light_blue, 2)
                    cv2.line(img_cumulative, (bbgt[2], bbgt[3]), (bbgt[4], bbgt[5]), light_blue, 2)
                    cv2.line(img_cumulative, (bbgt[4], bbgt[5]), (bbgt[6], bbgt[7]), light_blue, 2)
                    cv2.line(img_cumulative, (bbgt[6], bbgt[7]), (bbgt[0], bbgt[1]), light_blue, 2)
                    cv2.putText(img_cumulative, class_name, (bbgt[0], bbgt[1] - 5), font, 0.6, light_blue, 1,
                                cv2.LINE_AA)
                bb = [int(i) for i in bb]
                cv2.line(img, (bb[0], bb[1]), (bb[2], bb[3]), color, 2)
                cv2.line(img, (bb[2], bb[3]), (bb[4], bb[5]), color, 2)
                cv2.line(img, (bb[4], bb[5]), (bb[6], bb[7]), color, 2)
                cv2.line(img, (bb[6], bb[7]), (bb[0], bb[1]), color, 2)
                cv2.line(img_cumulative, (bb[0], bb[1]), (bb[2], bb[3]), color, 2)
                cv2.line(img_cumulative, (bb[2], bb[3]), (bb[4], bb[5]), color, 2)
                cv2.line(img_cumulative, (bb[4], bb[5]), (bb[6], bb[7]), color, 2)
                cv2.line(img_cumulative, (bb[6], bb[7]), (bb[0], bb[1]), color, 2)
    ...
    for tmp_file in gt_files:
        ...
        for obj in ground_truth_data:
            if not obj['used']:
                ...
                cv2.line(img, (bbgt[0], bbgt[1]), (bbgt[2], bbgt[3]), pink, 2)
                cv2.line(img, (bbgt[2], bbgt[3]), (bbgt[4], bbgt[5]), pink, 2)
                cv2.line(img, (bbgt[4], bbgt[5]), (bbgt[6], bbgt[7]), pink, 2)
                cv2.line(img, (bbgt[6], bbgt[7]), (bbgt[0], bbgt[1]), pink, 2)
